<template>
    ...
    <el-card style="margin:10px 0px;">
        <div v-show="scene == 0">
            ...
            <el-table border style="margin:10px 0px;" :data="attrArr">
                ...
                <el-table-column label="属性值名称" align="center">
                    <template #="{ row }">
                        <el-tag v-for="(item) in row.attrValueList" :key="item.id" style="margin:20px;">{{
                            item.valueName }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column label="操作" align="center" width="230px">
                    <template #="{ row }">
                        <el-button @click=" updateAttr(row)" icon="Edit" type="primary">编辑</el-button>
                        <el-popconfirm :title="`您确定要删除${row.attrName}属性吗?`" width="230px" icon="Delete"
        ...
        <div v-show="scene == 1">
            ...
            <el-button @click="addAttrValue" icon="Plus" type="primary"
                :disabled="attrParams.attrName ? false : true">添加属性值</el-button>
            <el-table border style="margin:10px -3px" :data="attrParams.attrValueList">
...
const save = async () => {
    ...
    if (result.code = 200) {
        ...
        let result1 = await reqAttr(categoryStore.c1Id, categoryStore.c2Id, categoryStore.c3Id);
        if (result1.code == 200) {
