<template>
  <el-form label-width="100px">
    <el-form-item label="SPU名称:">
      <el-input v-model="SpuParams.spuName" placeholder="请输入SPU名称" style="width: 220px"></el-input>
    </el-form-item>
    ...
    <el-form-item label="SPU销售属性:">
      <el-select style="width: 220px" v-model="saleAttrIdAndValueName" :placeholder="unSelectSaleAttr.length
      ...
      <el-button type="primary" icon="Plus" style="margin: 0px 20px" @click="addSaleAttr"
    ...
    <el-form-item>
      <el-table border :data="saleAttr">
        ...
        <el-table-column label="销售属性值" align="center">
          <template #="{ row }">
            <el-tag v-for="(item, index) in row.spuSaleAttrValueList" :key="row.id" closable style="margin: 0px 10px"
              @close="row.spuSaleAttrValueList.splice(index, 1)">
              {{ item.saleAttrValueName }}
            </el-tag>
            <el-input @change="toLook(row)" v-model="row.saleAttrValue" v-if="row.flag == true" placeholder="请输入属性值"
              size="small" style="width: 100px"></el-input>
            <el-button @click="toEdit(row)" v-else type="primary" size="small" icon="Plus"
              style="margin: 0px 10px"></el-button>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="120px">
          <template #="{ $index }">
            <el-button @click="saleAttr.splice($index, 1)" type="danger" size="small" icon="Delete"></el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-form-item>
    <div style="display: flex; justify-content: flex-end">
      ...
      <el-button :disabled="saleAttr.length > 0 ? false : true" @click="save" type="primary">
        保存
      </el-button>
    </div>
  </el-form>
