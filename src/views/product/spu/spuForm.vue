    ...
    <el-form label-width="100px">
        ...
        <el-form-item label="SPU销售属性:">
            <el-select style="width: 220px" v-model="saleAttrIdAndValueName"
                :placeholder="unSelectSaleAttr.length ? `还需选择${unSelectSaleAttr.length}个销售属性` : '无可选择的销售属性'">
        ...
        <el-form-item>
            <el-table border :data="saleAttr">
                ...
                <el-table-column label="销售属性值" align="center">
                    <template #="{ row }">
                        <el-tag v-for="(item, index) in row.spuSaleAttrValueList" :key="row.id" closable
                            style="margin:0px 10px;" @close="row.spuSaleAttrValueList.splice(index, 1)">
                            {{ item.saleAttrValueName }}
                        </el-tag>
                        <el-input @change="toLook(row)" v-model="row.saleAttrValue" v-if="row.flag == true"
                ...
                <el-table-column label="操作" align="center" width="120px">
                    <template #="{ $index }">
                        <el-button @click="saleAttr.splice($index, 1)" type="danger" size="small"
...
import { reqAddOrUpdateSpu, reqAllTrademark, reqSpuIamgeList, reqSpuHasSaleAttr, reqAllSaleAttr } from '@/api/product/spu';
