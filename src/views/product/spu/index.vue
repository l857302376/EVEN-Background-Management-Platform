        ...
        <div v-show="scene == 0">
            ...
            <el-table style="margin:10px 0px;" border :data="recordes">
                ...
                <el-table-column label="操作" align="center">
                    <template #="{ row }">
                        <el-button @click="addSku(row)" icon="Plus" title="添加SKU" type="success"
                            size="small"></el-button>
                        <el-button @click="updateSpu(row)" icon="Edit" title="修改SPU" type="warning"
                            size="small"></el-button>
                        <el-button @click="findSku(row)" icon="View" title="查看SKU列表" type="primary"
                            size="small"></el-button>
                        <el-popconfirm :title="`您确定要删除${row.spuName}吗?`" width="230px" icon="Delete"
            ...
            <el-pagination v-model:current-page="pageNum" v-model:page-size="pageSize" :page-sizes="[3, 6, 10]"
                ...
                @current-change="getSpu" @size-change="sizeChange" />
        </div>
...
const getSpu = async (pager = 1) => {
    pageNum.value = pager
    let result: HasSpuResponseData = await reqHasSpu(pageNum.value, pageSize.value, categoryStore.c3Id)
    if (result.code == 200) {
